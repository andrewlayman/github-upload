...
(* EMS FlexStor Page supply using only one frame buffer. *)
...
BEGIN
    ...
    ok := (FrameCache.FlexStorPageNumber = PageNo);

    IF (ok) THEN
        WhereAt   := FrameCache.PageFrameAddress;
        LockEntry := 1;
    ELSE
...
BEGIN
    ...
    ok := (FrameCache.EmsHandle = TheEmsHandle);
...
PROCEDURE InvalidateLockedPage();
BEGIN
    FrameCache.FlexStorPageNumber := NilEMSHandle;
...
BEGIN
    ...
    *)
    ...
    IF (LockEntry = 0) THEN
        LockEntry := 1;
        ok := (FrameCache.FlexStorPageNumber = NilEMSHandle);
    ELSIF (LockEntry = 1) THEN
        ok := TRUE;
    ELSE
        RETURN FALSE;
    END;
...
BEGIN                       (* XDiscardPage *)
    ...
    ELSE
        ...
        IF (FindLockedPageByHandle( TheEmsHandle, LockEntry )) THEN
            EMSUnLock( TheEmsHandle, FrameCache.FrameHandle );
            InvalidateLockedPage();
        END;
    ...
    ELSE
        ...
        *)
        ...
        IF (ok) THEN
            ...
            IF (LockEntry <> 0) THEN
                FrameHandle := FrameCache.FrameHandle; (* RSC 22-Aug-89 We already own the page. *)
            END;
